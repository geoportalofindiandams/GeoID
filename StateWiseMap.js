...
$.getJSON('Dams.json', function (DamItem) {
    ...
    $.each(DamItem, function (index, value) {
        DamList.push(value.State);
        AllDams = DamItem;
    });
    // Remove duplicates. We want unique State Name
    DamList = Array.from(new Set(DamList));

    // Fill the first dropdown with unique state
    $.each(DamList, function (index, value) {
        $('#select-state').append('<option value="' + value + '">' + value + '</option>');
    });
});
$('#select-state').change(function () {
    let StateName = this.options[this.selectedIndex].value;
    let filterData = AllDams.filter(function (value) {
        return value.State === StateName;
    });
    $('#select-dam')
        .empty()
        .append('<li value=""></li>');
...
var AllStatesBoundary = L.geoJSON(AllStatesBoundary, { color: 'black' });
AllStatesBoundary.addTo(map);
